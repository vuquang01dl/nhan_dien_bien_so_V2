...
void setup() {
  ...
  myservo.write(167);  // Đặt servo ở góc 90 độ (vị trí đóng)
...
void mo_barie() {
   ...
   // Mở barrier
  for (pos = 167; pos >= 70; pos -= 1) {  // Góc từ 90 đến 0
    myservo.write(pos);
  ...
  delay(3000);  // Giữ barrier mở trong 3 giây
     // Đóng barrier
  for (pos = 70; pos <= 167; pos += 1) {  // Góc từ 0 đến 90
    myservo.write(pos);
    delay(15);  // Chờ servo di chuyển
  }
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (receivedChar == 'B') {  // Nếu nhận được ký tự 'B'
